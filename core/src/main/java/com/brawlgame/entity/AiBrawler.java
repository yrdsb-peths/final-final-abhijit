...
public final class AiBrawler implements CombatTarget, Disposable {
    ...
    private static final float RUN_SPEED = 3.5f, WALK_SPEED = 1.5f, STRAFE_SPEED = 3.1f, STRAFE_RANGE = 3.6f;
    ...
    private static final float MELEE_DAMAGE = 6f, CRIT_MULT = 1.5f;
    ...
    private static final float RANGED_MIN = 3.8f, RANGED_MAX = 11f, RANGED_CD = 1.6f;
    private static final float GUN_DMG = 6f, GUN_SPEED = 15f, GUN_KB = 0.45f;

    private static final Color HURT_TINT = new Color(1f, 0f, 0f, 1f);       // pure red
    private static final Color BODY_TINT = new Color(1f, 0.78f, 0.72f, 1f); // warm so it reads as the rival
    ...
    private float facingDeg, maxHealth = 50f, health = 50f; // tanky: the potato gun chips it, doesn't one-shot
    ...
    private final ColorAttribute bodyTint = ColorAttribute.createDiffuse(BODY_TINT);
    ...
    // ---- held diamond sword (body-anchored, swings on attack) ----
    ...
    public AiBrawler(Texture skin, float x, float z, float minX, float maxX, float minZ, float maxZ) {
        ...
        pos.set(x, 0f, z);
        for (Material m : instance.materials) m.set(bodyTint); // permanent warm tint vs the player
        tinted = false;
    ...
    public void update(float delta, Player player) {
        ...
        if (attacking) {
            ...
            if (attackTimer <= 0f) {
                ...
                if (dist < MELEE_HIT_RANGE) {
                    dir.set(dx, 0f, dz).nor();                  // push the player away from the bot
                    player.applyHit(MELEE_DAMAGE * (crit ? CRIT_MULT : 1f), dir, crit ? KB_CRIT : KB_TO_PLAYER);
                }
            }
            ...
            state = State.CHASE;
            // Opportunistic potato fire while closing.
            if (onGround && dist > RANGED_MIN && dist < RANGED_MAX && rangedCooldown <= 0f) {
                fireGun(dx, dz, dist);
                rangedCooldown = RANGED_CD;
            }
            if (dist > STRAFE_RANGE) {
                setSteer(dx, dz, RUN_SPEED);                    // run straight in
            } else {
                strafe(delta, dx, dz);                          // circle-strafe / juke up close
            }
        } else {
            state = State.WANDER;
            wanderTimer -= delta;
        ...
        animator.update(delta, horizSpeed, state == State.CHASE || state == State.ATTACK, false, onGround, null);
        applyTint();
        hearts.update(delta);
        updatePotatoes(delta, player);
    }
    ...
    public void onHit(float damage, Vector3 fromDir, boolean crit) {
        if (dying || gone) return;
        float dealt = ArmorStats.reduce(damage, gear); // iron armour soaks part of the player's hit
    ...
    /** Place + swing the held sword: forward-down at rest, raised overhead in wind-up, slashed on lunge. */
    private void anchorSword(float windup) {
        float pitch;
        if (attacking) pitch = MathUtils.lerp(70f, 165f, windup);              // raise overhead
        else if (lungeTimer > 0f) pitch = MathUtils.lerp(165f, 15f, 1f - lungeTimer / ATTACK_LUNGE); // slash down
        else pitch = 70f;                                                       // ready, out front
        sword.transform.set(instance.transform)
            .translate(0.42f, 1.32f, -0.05f)   // right-hand area (model right = +X)
    ...
    private void applyDeathTransform() {
        ...
        instance.transform.setToRotation(Vector3.Y, facingDeg)
            .setTranslation(pos)
            .rotate(Vector3.Z, roll);                   // local Z = sideways → falls to the side
        anchorSword(0f);                                 // sword follows the toppling body
    }
    ...
    private void applyTint() {
        ...
        tinted = hurt;
        for (Material m : instance.materials) m.set(hurt ? redTint : bodyTint);
    }

    public void render(ModelBatch batch, Environment env) {
        if (!gone) {                               // keep drawing through the death tip-over
            batch.render(instance, env);
            armor.render(batch, env, instance);   // worn iron armour over the rig
            batch.render(sword, env);              // held diamond sword
            hearts.render(batch);
        }
